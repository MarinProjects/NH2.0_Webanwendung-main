...
@Component({
  selector: 'app-anpassung-rgo-halbjahr1',
  templateUrl: './anpassung-rgohalbjahr1.component.html',
  styleUrls: ['./anpassung-rgohalbjahr1.component.css']
})
export class AnpassungRGOHalbjahr1Component {
  form: FormGroup;
  ...
  constructor(
    ...
    this.form = this.fb.group({
      ...
      sollGesamtversorgungAngepasstWerden: [true, Validators.required],
  ...
  onSubmit(): void {
    ...
    this.personService.submitRGOHalfYear1Adjustment(payload).subscribe({
      next: (res) => {
        alert(res.message);
  ...
  private downloadCSV(details: any[]): void {
    const header = 'Personalnummer,Name,BetrRente letztes Halbjahr,BetrRente neues Halbjahr\n';
    const rows = details.map(d =>
      `${d.personalnummer},"${(d.name || '').replace(/"/g, '""')}",${this.f2(d.betrRenteLastHalf)},${this.f2(d.betrRenteNewHalf)}`
    );
    ...
    link.download = `Kontrolliste_RGO_Halbjahr1_${currentDate}.csv`;
